...
//					int frameIndex = shapeIndex*3 + styleIndex;
		...
		PrefabAssetType prefabAssetType = PrefabAssetType.NotAPrefab;
		PrefabInstanceStatus prefabInstanceStatus = PrefabInstanceStatus.NotAPrefab;
	#else
...
#if !SHFX_DEBUG
	#if UNITY_2018_3_OR_NEWER
			prefabAssetType = PrefabUtility.GetPrefabAssetType(this.target);
			prefabInstanceStatus = PrefabUtility.GetPrefabInstanceStatus(this.target);
	#else
...
#if !SHFX_DEBUG
	#if UNITY_2018_3_OR_NEWER
			if(prefabAssetType != PrefabAssetType.NotAPrefab && prefabInstanceStatus == PrefabInstanceStatus.NotAPrefab)
	#else
			if(prefabType == PrefabType.Prefab)
	#endif
			{
	#if UNITY_2018_3_OR_NEWER
				EditorGUILayout.HelpBox("This tool cannot be used on Prefabs to prevent data loss.\nPlease either:\n- Open the Prefab for editing\n- Or drop an instance of the prefab into your Scene, do your modifications on it, and then manually Apply the changes on the Prefab or create a new Prefab", MessageType.Warning);
	#else
